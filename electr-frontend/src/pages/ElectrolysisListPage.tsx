...
import { getElectrolysisList, getOrderById } from '../api/electrolysisApi';
...
export const ElectrolysisListPage = () => {
    ...
    const [searchTitle, setSearchTitle] = useState(() => sessionStorage.getItem('electro_search') || '');
    const [minVolt, setMinVolt] = useState(() => sessionStorage.getItem('electro_min') || '');
    const [maxVolt, setMaxVolt] = useState(() => sessionStorage.getItem('electro_max') || '');

    const fetchItems = () => {
        setLoading(true);
        getElectrolysisList({ title: searchTitle, min_voltage: minVolt, max_voltage: maxVolt })
            .then(data => setItems(data.items || []))
            .catch(err => console.error("Failed to fetch", err))
            .finally(() => setLoading(false));
    };

    // 1. Фильтры + Debounce (Обновление списка при вводе поиска)
    useEffect(() => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            getOrderById(1)
                .then(orderData => {
                    if (orderData) {
                        // Чисто технический лог, как ты просил
                        console.log(`[Polling Success] Order #1 -> Status: ${orderData.status}, CalcValue: ${orderData.calculated_value}`);
                    }
                })
    ...
    }, []); // Пустой массив зависимостей, чтобы таймер не пересоздавался лишний раз
    

    const crumbs = [{ label: 'Услуги электролиза', active: true }];
    ...
    return (
        <div className="page-container">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', marginBottom: '30px' }}>
                <input className="search-input" style={{ width: '600px' }} type="text" placeholder="Поиск..." value={searchTitle} onChange={(e) => setSearchTitle(e.target.value)} />
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <label>Напряжение (В):</label>
                    <input type="number" placeholder="от" className="search-input" style={{ width: '100px' }} value={minVolt} onChange={(e) => setMinVolt(e.target.value)} />
                    <span>—</span>
                    <input type="number" placeholder="до" className="search-input" style={{ width: '100px' }} value={maxVolt} onChange={(e) => setMaxVolt(e.target.value)} />
                </div>
            </div>
                    ...
                    {items.length > 0 ? items.map(item => {
                        return (
                            <div className="custom-card" key={item.id}>
                                ...
                                <h3>{item.title}</h3>
                                <p>{item.text ? (item.text.length > 100 ? item.text.slice(0, 100) + '...' : item.text) : ''}</p>
                                ...
                                {/* 
                                    Здесь раньше были блоки с расчетом и статусом. 
                                    Они удалены полностью. 
                                    Интерфейс чист.
                                */}

                                <Link to={`/electrolysis/${item.id}`} className="card-btn">Подробнее</Link>
                            </div>
                        );
                    }) : (
            ...
            <div style={{textAlign: 'center', margin: '30px 0', color: '#888'}}>&copy; Иванов Андрей ИУ5-53Б</div>
        </div>
    );
};
